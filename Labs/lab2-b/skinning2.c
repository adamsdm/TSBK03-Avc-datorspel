...
void DeformCylinder()
{
  int row, corner;
	...
	mat4 Mben[kMaxBones];
	mat4 MbenI[kMaxBones]; 

	// ====== Init Mben ====== //
	for(int i=0; i<kMaxBones; i++){
		mat4 Tb = T(g_bonesRes[i].pos.x, g_bonesRes[i].pos.y, g_bonesRes[i].pos.z);
		mat4 Rb = g_bonesRes[i].rot;
		Mben[i] = Mult(Tb, Rb);
	}

	// ====== Init MbenI ====== //
	for(int i=0; i<kMaxBones; i++){
		MbenI[i] = InvertMat4(Mben[i]);
	}

	// ====== Init M ====== //
	Mprim[0] = Mult(T(g_bonesRes[0].pos.x, g_bonesRes[0].pos.y, g_bonesRes[0].pos.z), g_bonesRes[0].rot);
	M[0] = Mult(Mprim[0], MbenI[0]);
	
	for(int i=1; i<kMaxBones; i++){
		vec3 Tb1 = SetVector(0.0, 0.0, 0.0);

		mat4 dTb = T(g_bonesRes[i].pos.x - g_bonesRes[i-1].pos.x, g_bonesRes[i].pos.y - g_bonesRes[i-1].pos.y, g_bonesRes[i].pos.z - g_bonesRes[i-1].pos.z);
		mat4 Rb = g_bonesRes[i].rot;
		Mprim[i] = Mult(Mprim[i - 1], Mult(dTb, Rb));

		M[i] = Mult(Mprim[i], MbenI[i]);
